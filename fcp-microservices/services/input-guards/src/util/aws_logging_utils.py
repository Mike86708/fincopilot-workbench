...
class LogType(Enum):
    '''     
    Enum for log type.
    Defines various types of logs, such as SERVICE_INPUT, SERVICE_OUTPUT, LLM_DETAIL, FUNCTION_INPUT, FUNCTION_OUTPUT, and STATUS.
    '''
...
def log_cloudwatch(log_type: LogType, message: str, args: dict, prompt_id: str = None,conversation_id: str = None,session_id: str = None, log_level: LogLevel = LogLevel.INFO):    
    '''
    ...
    if not AWS_LOGGING['enabled']:
      ...
      # Ensure service_name and source_name are included
    aws_log['service_name'] = config['app']['name']
    aws_log['source_name'] = 'query_controller'
    
    # Include prompt_id if provided
    if prompt_id:
        aws_log['prompt_id'] = prompt_id
    
    if conversation_id:
        aws_log['conversation_id'] = conversation_id
        
    if session_id:
        aws_log['session_id'] = session_id        
